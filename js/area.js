        ...
        if (false === pointClicked) {
            ...
            if (p.COUNTYNAME) {
                ...
                if (cityMeta[cityKey]) {
                    ...
                    message += '<tr><th scope="row">面積</th><td>' + cityMeta[cityKey].areasize + '</td></tr>';
                    message += '<tr><th scope="row">比率</th><td>' + cityMeta[cityKey].rate + '(每平方公里)</td></tr>';
                    message += '</tbody></table>';
                    ...
                    message += '<tr><th scope="row">面積</th><td>' + sizePool[cityKey] + '</td></tr>';
                    message += '<tr><th scope="row">比率</th><td>0(每平方公里)</td></tr>';
                    message += '</tbody></table>';
                }
            } else {
                message += '<table class="table table-dark"><tbody>';
    ...
    $.get('https://kiang.github.io/data.moi.gov.tw/json/population/city/2022/11.json', {}, function (c) {
        for (code in c) {
            ...
            if (cityMeta[c[code].area]) {
                cityMeta[c[code].area].areasize = c[code].size;
                if (cityMeta[c[code].area].confirmed > 0 && cityMeta[c[code].area].areasize > 0) {
                    cityMeta[c[code].area].rate = Math.round(cityMeta[c[code].area].confirmed / cityMeta[c[code].area].areasize * 100) / 100;
                    rateList.push({
                        area: c[code].area,
                        areasize: cityMeta[c[code].area].areasize,
        ...
        listTable += '<tr><th>區域</th><th>確診數</th><th>面積(平方公里)</th><th>比率</th></tr>';
        for (k in rateList) {
            listTable += '<tr>';
            listTable += '<td>' + rateList[k].area + '</td>';
            listTable += '<td>' + rateList[k].cases + '</td>';
            listTable += '<td>' + rateList[k].areasize + '</td>';
    ...
    for (c1 in c) {
        for (c2 in c[c1]) {
            ...
            if (!cityMeta[cityKey]) {
                ...
                if (sizePool[cityKey]) {
                    cityMeta[cityKey].areasize = sizePool[cityKey];
                }
            }
            cityMeta[cityKey].confirmed = c[c1][c2];
            cityMeta[cityKey].increaseRate = r.rate[c1][c2];
            cityMeta[cityKey].increase = r.increase[c1][c2];
            if (cityMeta[cityKey].areasize > 0) {
                cityMeta[cityKey].rate = Math.round(cityMeta[cityKey].confirmed / cityMeta[cityKey].areasize * 1000000) / 100;
            }
